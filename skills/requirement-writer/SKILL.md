...
완성된 요구사항 정의서를 `{프로젝트 디렉토리}/output/requirement.md`에 저장함.
...
**질문 1: 기본 정보**
- 플러그인명: 플러그인의 고유 이름 (예: abra, code-reviewer)
- 목적: 한 문장으로 표현하는 플러그인의 핵심 목적
- 대상 도메인: 적용 분야 (예: 교육, 개발 자동화, 비즈니스)
- 대상 사용자: 주요 사용자 유형 (예: 개발자, 기획자)

**질문 2: 핵심기능**
- 플러그인이 수행할 핵심 기능 목록 (각 기능에 간략한 설명 포함)
- 예시 제시: "PR 분석: PR의 변경 파일, 커밋 히스토리, 영향 범위 자동 분석"

**질문 3: 사용자 플로우**
- 사용자가 플러그인을 사용하는 단계별 흐름
- 예시 제시: "Step 1. PR 지정: 리뷰할 PR 번호 또는 브랜치 지정"
...
참조: `resources/guides/plugin/plugin-dev-guide.md`의
"에이전트 유형 참조" 섹션

| 기능 특성 | 추천 에이전트 유형 | 근거 |
|----------|------------------|------|
| 코드 작성/수정 필요 | executor | 구현 전문가 |
| 구조 분석/설계 결정 필요 | architect | 분석/설계 전문가 |
| 코드베이스 탐색/검색 필요 | explorer | 탐색 전문가 |
| 전략적 계획 필요 | planner | 계획 수립 전문가 |
| 산출물 검토/비평 필요 | reviewer | 검토 전문가 |
| 외부 문서/자료 조사 필요 | researcher | 조사 전문가 |
| 문서 작성 필요 | writer | 문서 작성 전문가 |
| 위 유형에 해당 없음 | 도메인 특화 유형 정의 | 커스텀 |

분석 절차:
1. 각 핵심기능이 필요로 하는 역할 유형 매칭
2. 사용자 플로우의 Step 간 역할 전환 패턴 분석
   에이전트 분리 필요성 판단
...
1. `output/` 디렉토리 존재 여부 확인
   미존재 시 자동 생성
2. `output/requirement.md` 파일 존재 여부 확인
   존재 시 AskUserQuestion으로 덮어쓰기 확인
3. `{프로젝트 디렉토리}/output/requirement.md`에 저장
4. 사용자에게 완료 안내:
   - 저장 경로 안내
...
| `output/requirement.md` 이미 존재 | AskUserQuestion으로 덮어쓰기 여부 확인 |
