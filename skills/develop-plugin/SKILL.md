...
4. **공유자원 매핑 보강** (Step 3 자동 복사가 정확하게 동작하도록):
   - 각 office 가이드·템플릿이 어떤 에이전트·스킬에 속하는지 개발 계획서 공유자원 표에 명시
   - 예시:
     ```
     | 가이드 | pptx-build-guide | {DMAP}/.../office/pptx-build-guide.md | → agents/pptx-spec-writer/references/ |
     | 가이드 | xlsx-build-guide | {DMAP}/.../office/xlsx-build-guide.md | → skills/generate-xlsx/references/ |
     | 템플릿 | pptx-spec-writer-AGENT | {DMAP}/.../office/pptx-spec-writer-AGENT.md | → agents/pptx-spec-writer/AGENT.md (시작점) |
     | 템플릿 | pptx-builder-SKILL | {DMAP}/.../office/pptx-builder-SKILL.md | → skills/generate-pptx/SKILL.md (시작점) |
     ```
   - 이 매핑이 정확해야 Step 3에서 적절한 위치로 복사됨
...
- 각 자원 유형에 따라 아래 위치에 복사
  ...
  ```
- '도구' 유형의 자원은 복사한 도구소스를 분석하여 환경변수(.env)파일을 적절한 위치에 생성           
...
- **pptx만**: `{PLUGIN_DIR}/agents/pptx-spec-writer/AGENT.md` 생성
  ...
  - 가이드 참조: Phase 2 Step 0에서 `pptx-build-guide`를 `pptx-spec-writer` 에이전트의
    ...
    `{PLUGIN_DIR}/agents/pptx-spec-writer/references/pptx-build-guide.md`로 자동 복사됨
...
- `{PLUGIN_DIR}/gateway/runtime-mapping.yaml` 기반 tier-mapping의 모델을 최신 버전으로 업데이트하고 사용자에게 확인 요청   
- 커스텀 도구에 필요한 환경변수(.env) 파일 생성 및 설정: API Key 등 민감 정보는 사용자에게 입력 요청하여 설정
