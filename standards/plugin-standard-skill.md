...
| 스킬이 직접 수행 | 에이전트에 위임 (Agent 도구) | 다른 스킬에 위임 (Skill 도구) |
|------------------|---------------------------|-------------------------------|
...
| 상황 | 위임 대상 | 도구 | 이유 |
|------|----------|------|------|
| 단일 작업 실행 (코드 작성, 분석 등) | Agent | `Agent` | 에이전트가 자율적으로 완수 |
...
| 항목 | `→ Agent:` (Agent 도구) | `→ Skill:` (Skill 도구) |
|------|------------------------|--------------------------|
| 필수 항목 | 5항목 | 3항목 |
...
|----------------|----------|------|:----:|
| 기획 수립 | `/plan` | 요구사항 도출 워크플로우 | **필수** |
| 개발 계획 수립 | `/ralplan` | 기술 합의 워크플로우 | **필수** |
| 기능 구현 | `/ralph` | 완료 보장 실행 워크플로우 | **필수** |
| 빌드 오류 수정 | `/build-fix` | 최소 수정 원칙 | **필수** |
| QA/검증 | `/ultraqa` | QA 순환 워크플로우 | **필수** |
| 계획 검토 | `/review` | 비평 전문 워크플로우 | **필수** |
| 심층 분석/디버깅 | `/analyze` | 체계적 분석 절차 | **필수** |
| 코드베이스 탐색 | `/deepsearch` | 심층 검색 방법론 | **필수** |
| 코드 리뷰 | `/code-review` | 종합 리뷰 체크리스트 | **필수** |
| 보안 검토 | `/security-review` | 보안 감사 워크플로우 | **필수** |
| 리서치/조사 | `/research` | 병렬 조사 방법론 | **필수** |
