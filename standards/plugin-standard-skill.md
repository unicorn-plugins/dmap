...
| 설정스킬 | Setup | 직결형 | 설치 · 설정 마법사 제공 | setup |
| 지휘자스킬 | Orchestrator | 위임형 | 워크플로우 조율, 병렬 실행, 분석, 리뷰 | analyze, tdd |
| 핼프스킬 | Help | 직결형 | 사용 도움말 제공 | help |

### 스킬 실행 경로

스킬은 분류에 따라 세 가지 실행 경로 중 하나를 따름.

| 분류 | 의미 | 해당 유형 | Agent 계층 |
|----------|------|----------|-----------|
| **위임형** | LLM 추론·자율 판단이 필요한 작업 → Agent에 위임 | Router, Orchestrator | 경유 (필수) |
| **직결형** | 절차적·결정론적 작업 → Gateway 도구 직접 사용 | Setup, Help | 생략 |
