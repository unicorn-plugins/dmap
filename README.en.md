...
Plugins generated by DMAP support all four runtimes below via an SSOT (`agents/{name}/`) + **thin per-runtime pointer adapter** approach.
Agent definitions are authored once; `develop-plugin` automatically generates runtime-specific stubs.

| Runtime | Adapter Path | Format | Example Models | Invocation |
|---------|--------------|--------|----------------|------------|
| Claude Code / CoWork | `.claude/agents/{name}.md` | Markdown + YAML frontmatter | `opus`, `sonnet`, `haiku` | `Agent(subagent_type=FQN, ...)` |
| Cursor | `.cursor/agents/{name}.md` | Markdown + YAML frontmatter | `opus`, `sonnet`, `haiku` | `/{agent}` or natural-language delegation |
| Codex | `.codex/agents/{name}.toml` | TOML + `developer_instructions` | `gpt-5.4`, `gpt-5.4-mini`, etc. | `spawn_agents_on_csv` or natural language |
| Antigravity | `.antigravity/agents/{name}.md` | Markdown + YAML frontmatter | `opus`, `sonnet`, `haiku` | Manually load via Manager UI (programmatic API uncertain) |

> **Core design**: The SSOT lives only in `agents/{name}/`. Each runtime adapter stub is a thin pointer
> that merely instructs the agent to "read the three SSOT files and act accordingly."
> Model versions are resolved from the 4-runtime tier mapping in `gateway/runtime-mapping.yaml`
> and written into the stub frontmatter.
> Details: [agent-runtime-adapters.md](resources/guides/agent-runtime-adapters.md)
...
|-------|------|-------------|
| **Current** | Multi-runtime adapter support | Auto-generated pointer adapters for 4 runtimes: Claude Code / Cursor / Codex / Antigravity |
| **Short-term** | Runtime validation and coverage expansion | Validate Antigravity programmatic API, add adapters for Gemini CLI and other runtimes |
